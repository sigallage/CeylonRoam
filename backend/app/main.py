...
LOCATION_TO_PROVINCE = {
    ...
    "Bentota": "Western",           # Gampaha District
    ...
    "Sinharaja": "Sabaragamuwa",    # Ratnapura/Galle Districts (border area)
...
def _build_theme_location_map(provinces: list[str]) -> Dict[str, list[str]]:
    ...
    for theme_name, theme_data in THEME_LIBRARY.items():
        ...
        # Debug logging
        if provinces:
            print(f"Theme '{theme_name}': {len(filtered_locs)} locations in {provinces} -> {filtered_locs}")
...
def _generate_itinerary(
    *,
    ...
    for day_index in range(day_total):
        ...
        location = unused_locations[len(used_locations) % len(unused_locations)]
